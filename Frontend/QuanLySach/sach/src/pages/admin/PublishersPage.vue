...
export default {
  data() {
    return {
      apiUrl: 'http://localhost:8080/api/publisher',
      form: {
        id: null,
        name: '',
  ...
  methods: {
    async loadData() {
      ...
      try {
        ...
        this.list = rows.map((item) => ({
          id: item.id,
          name: item.name || '',
          totalPublishedBooks: Number(item.totalPublishedBooks ?? item.total_published_books ?? 0),
    ...
    async addItem() {
      ...
      try {
        await axios.post(
          ...
          {
            name: this.form.name,
            totalPublishedBooks: Number(this.form.totalPublishedBooks),
        ...
        this.message = 'Không thể thêm nhà xuất bản'
      }
    },
    ...
    async remove(id) {
      ...
      try {
        ...
        this.message = 'Không thể xóa nhà xuất bản'
      }
    },
    ...
    edit(item) {
      ...
      this.form = {
        id: item.id,
        name: item.name,
        totalPublishedBooks: item.totalPublishedBooks,
    ...
    validate() {
      ...
      if (Number(this.form.totalPublishedBooks) < 0) {
        this.errors.totalPublishedBooks = 'Tổng số sách phát hành phải lớn hơn hoặc bằng 0'
      }
      if (Number(this.form.averageLoanDays) <= 0) {
        this.errors.averageLoanDays = 'Số ngày mượn trung bình phải lớn hơn 0'
      }
...
<template>
  <div class="container py-4">
    <h4 class="mb-3">Quản lý nhà xuất bản</h4>

    <div class="row g-3">
      <div class="col-12 col-lg-4">
        <div class="card">
          <div class="card-header">
            <strong>{{ isEditing ? 'Cập nhật nhà xuất bản' : 'Thêm nhà xuất bản' }}</strong>
          </div>
          <div class="card-body">
            <div class="mb-3">
            ...
            <div class="mb-3">
              <label class="form-label">Tổng số sách phát hành</label>
              <input
                v-model.number="form.totalPublishedBooks"
                type="number"
                min="0"
                class="form-control"
              />
              <div v-if="errors.totalPublishedBooks" class="text-danger small mt-1">
                {{ errors.totalPublishedBooks }}
              </div>
            </div>
            ...
            <table class="table table-bordered">
              <thead>
                <tr>
                  <th>ID</th>
                  <th>Tên</th>
                  <th>Tổng số sách phát hành</th>
                  <th>Số ngày mượn trung bình</th>
                  <th>Trạng thái</th>
                  <th>Thao tác</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="item in list" :key="item.id">
                  <td>{{ item.id }}</td>
                  <td>{{ item.name }}</td>
                  <td>{{ item.totalPublishedBooks }}</td>
